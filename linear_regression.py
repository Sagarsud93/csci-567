...
def mean_absolute_error(w, X, y):
    ...
    err = np.abs(np.matmul(X, w)-y).mean()
    return err
...
def linear_regression_invertible(X, y):
    ...
    while invertible == False:
        ...
        if np.amin(np.abs(ev)) < 0.00001:
            XtX += np.eye(XtX.shape[0],XtX.shape[1])*0.1
        else:
            invertible = True
                
    w = np.matmul(np.matmul(np.linalg.inv(XtX), np.transpose(X)), y)
    return w
...
def regularized_linear_regression(X, y, lambd):
  ...
  #####################################################		
    XtX = np.matmul(np.transpose(X), X)
    w = np.matmul(np.matmul(np.linalg.inv(XtX + np.eye(XtX.shape[0],XtX.shape[1]) * lambd), np.transpose(X)), y)
    return w
